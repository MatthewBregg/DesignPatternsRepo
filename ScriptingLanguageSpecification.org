...
(image "foo.jpg")
#+END_SRC
A image decal can be given a third argument, to determine some extra behavior.
...
- IF S, stretch to fit.
- All the options aside S, which does need to, will add transparent padding to return an image the size 
  render desires.
- The default is "S", so calling with the "S" argument is the same as not having a third argument
#+BEGIN_SRC 
(image "filepath.[jpg|png|etc]" "W")
(image "foo.jpg" "S")
#+END_SRC
- Rectangle Decal
#+BEGIN_SRC 
