...
** Overview
The Card object is constructed by five attributes: two layouts for front and back, a name, a width and a height. The ;ayout is of object ComponentLayout, which could be composed of other layouts or Decals. A Decal is the base object that can be put on a Card object. A Decal can be made of either an Image, a Text, a DynamicDecal or a Shape object.
...
This class is not part of any design patterns explicitly, but lots of design patterns will be used in its subclasses and by overriding its method.
...
- *Purpose:* 
This is a concrete class that will be able to hold an array of children of types ComponentLayout. The array will be filled with zero or many references to the LayoutPositionPair objects.
- *Interaction:*  
...
Basic number manipulation is provided by the NumberOperationBuilder, which is paramatized with a command.
...
*** Why we used?
Basic number manipulation can be done with some op, being applied to two numbers, resulting in a third.
Some op can be division, adition, etc.
To make a whole new builder for each one would be a class explosion, so instead, we paramatize the builder with the correct some op. 
*** Consequences
- Only one builder is needed to handle any number operation of the form a someop b = c.
