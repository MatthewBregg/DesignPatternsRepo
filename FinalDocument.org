...
The builders are loaded from an xml file, which as a name, and path to each visitor to be loaded.
** Strategy Pattern
...
as the script evaluator provides the interface for actually evaluating
