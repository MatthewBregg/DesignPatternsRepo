...
** Indonesia
- The card representation being two systems
  ...
  - Rather than each leaf being subclassed for each possible visual form, 
    we separate the systems, and have leaf message forward to decal
    which is an abstract class that represents something visual.
...
** Antiquity 
- Idea of layouts have shapes.
- Shapes not being at all tied to decals
- The way that layouts are rendered. This behavior was moved out into a visiter.
...
- We both came to a similar composite representation of a card.
  ...
  - Both had very similar functions, we took the layout name, but our layouts are mostly their panels. 
- Using the visiter pattern in parsing.
