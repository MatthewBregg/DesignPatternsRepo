...
Minor note, Driver for BuilderScriptObject is actually the visiter pattern it does, and using this many builders is a VBI.
* Overall System, Subsystem Components from each Subteam
...
The driver then will let Options get the correct file, and then get the Lexer from the ScriptEvaluator, tokenize the file, and evaluate the tokens.
Then, it will take the RenderedCard list and give each card to output, with options.
Output will then write the card to disk in the specified output file format.
The program is then done and will exit, printing any information left in the logger. 

Note that during each step, the Driver checks the logger for errors, and decides if it should abort. 
...
This allows for the language to add functionality at runtime by adding a feature to text file without having to recompile. This effectively adds keywords to the scripting language.

* What we took from each project
** Indonesia
- The card representation as two systems:
  - The composite structure, and the visual representation for each position.
  - Rather than each leaf being subclassed for each possible visual form, 
    we separate the systems, and have leaf messages forwarded to Decal,
    which is an abstract class that represents visual elements.
- The scripting language itself, although it was greatly extended. 
- Tokenizing the language through the use of a recursive descent parser.
- Most of the parsing portion.
  - Antiquities parsing specs were very high level, and we decided to focus 
    on ours, which were more in detail,
    and could be "implemented by a monkey"
  - This includes the builders. 
    

** Antiquity
- Idea of layouts having shapes.
- Shapes being decoupled from decals.
- The way that layouts are rendered. This behavior was moved out into a Visitor.
- Shapes
- Color decal
- The scripting language was much more simplified and easier to understand for non-programmers, but was less extensible than Indonesia's Sheme-based script.

** Both
- We both came to a similar Composite representation of a card.
  - Indonesia used layouts
  - Antiquity used panels
  - Both had very similar functions, we chose the layout name, but layouts are effectively panels. 
- Using the Visitor pattern in parsing.
  
** Neither, new ideas.
- Standard library of functions implemented in language, to allow easy extension of new convenience features. 
- If statements, which brings us to turing completeness.
- Decal decorators for basic image operations. 
- Plugins, dynamically loading builders.
- Loading builders from an xml file into the environment.
* Turing Complete
To quote Wikipedia, 
#+BEGIN_QUOTE
...
**** Decal Rendering Visitors
Each Visitor knows how to render either a text, image, or color decal for each shape. 
The visitor is made by said decal via an FM, and then is accepted by the shape to do the rendering. 
Parameterized with anything the visitor needs for rendering.
...
*** Factory Method
**** Why we used?
\newline Each non decorating, and non dynamic decal has a specific visitor which it must make. The super is the client. 
**** What we gained?
\newline Customizes the render method with which object should be created, enabling the superclass to implement it.
...
**** Consequences
\newline The decal decorators, and dynamic decals, don't have a visitor to return, and thus, have to return null. This is bad. 

*** Visitor
**** The ShapeVisitor used by Composite Layout
***** Why we used?
\newline A composite layout has some shape, but doesn't know what shape, nor should it!. 
It also shouldn't know how to position things inside a shape.
Instead, the visitor is accepted by the shape, and thus knows what shape it has, and can then properly place items.
***** What we gained?
\newline The composite layout need not know how to position things inside a shape, but still can.
***** Consequences
Visitor can get unwieldy, and a bit confusing.
\newline If we add a new shape, all visitors need be updated.
**** The ShapeVisitor used by Decal 
***** Why we used?
\newline A Decal knows what it has that represents some visual thing, an image, text, or shape, but not how to render it in a circle, rectangle, or other shape, or even what shape it has.
That is left to the visitor to determine, and do the rendering.
***** What we gained?
\newline Decal don't care about what shape they are told to render in, just make their visitor, and have the visitor visit and render in that shape. 
***** Consequences
\newline A visitor for each DecalWithFM implementor.
If we add a new shape, all visitors need be updated.

*** Decorator
**** Why we used?
\newline We found that we wanted to do certain actions to decals, that would result in new decals be added.
...
**** What we gained?
\newline We can do basic image manipulation by chaining decorator decals, and then using those decorators like normal decals. 
**** Conseqences
\newline Potentially inefficient. 



*** Composite
**** Why we used?
\newline Our team decided to uses the Composite pattern because it offers an easy way to represent hierarchies and also it offers an easy way to treat individual objects and composite objects the same way. Our system allows for a layout component to have layouts or simpler objects such as decal.
**** What we gained?
...
*** Iterator
**** Why we used?
\newline We decide to use this in order to allow an easy and simple way to access all the elements that will be used to represent a card object.
**** What we gained?
...
*** Proxy
**** Why we used?
\newline Proxy pattern is used very lightly and only just one part of it. It is being used in the DynamicDecal class. The Dynamic Decal is the RealSubject, which will carry (execute) the request forwarded to it. The request would be to receive a decal given a string.
**** What we gained?
...
**** Why we used?
\newline The adapter pattern is used for our Image class. Our image class will not be able to directly conform to the Image interface provided by the Java libraries. Therefore, we need to create a JavaImageAdapter class to adapt the interface we want with the interface we have.
**** What we gained?
...
The builders are loaded from an xml file, which has a name, and path to each visitor to be loaded.
** Strategy Pattern
The OutputFormat enum, and Output use the strategy pattern, with each being the strategy, and context, respectively.
This is because output shouldn't be burdened with every possible output format in a switch statement or something of the like.
We want to be able to easily add more formats, and to avoid the Output class from becoming bloated.
Thus, the output class will delegate converting an image into the correct output format, and then writing that to disk, for the enum it holds. 
** Driver Class
- *Purpose:* 
...
The Driver will also load any builders requesting to be loaded, and place them in the ScriptEvaluator's environment.
- *Design Patterns used:*
The Strategy pattern is used when outputting the image to the proper file format. The strategy, saveFileToDisk() is implemented in multiple subclasses for each file type.
...
** Options Class
- *Purpose:* 
To keep track of different variables of the deck found in the config file, such as input/output/logger directories or card output  types(png, jpg...).
- *Interaction:*  
The Options class is created in the driver after parsing the config file. It's also used by the logger and output class both use the options class for knowing where to write to. 
- *Design Patterns it belongs:*
No design patterns used in it.

** Output Class
- *Purpose:* 
Where all the cards in the deck are written to when saved.
- *Interaction:*  
The Driver will create the output class after evaluating the script, based on the options class. This is where all the cards will be written to after they are all created, rendered, and saved.
- *Design Patterns used:*
No design patterns used in it.

* ScriptEvaluator system 
This system includes all the Builders and the Lexer. Whenever I say Builders, I mean subclasses of ScriptObjectBuilder.

** Pluggable Class Adapter
The Lexer interface, and the ScriptEvaluator both act as pluggable adapters. 

Since we decided that we shouldn't hard code in a certain textual syntax, 
we decided to implement an interface, so that any language that can fit 
into our token structure can be used by implementing the Lexer interface.
If evaluating those tokens is changed by the language, which it likely is,
the ScriptEvaluator provides the interface for actually evaluating
the language, which also should be implemented. 
...
Avoids need to cast when retrieving a ScriptObject from the environment, the ScriptObject tells the visitor what is being added. 
*** What we gained?
...
- The Builder is parameterized with some number command to run upon creating a number value. 
...
**** Why we used?
\newline Used to enable expression tokens to be made up of subexpressions, and for any piece of an expression to be treated the same when iterating through it, regardless of if it is a Variable, Expression, or Atom literal.
**** What we gained?
...
*** Why we used?
To enable storing, and retrieving builders, some of which may be parameterized. For example, the function builders store the code to run when the function is called. 
*** What we gained?
